...
/* Fast path: ROM/RAM/VRAM (<0xa000) read straight from the array; only
 * I/O (>=0xa000) needs the machine_rd trap. Avoids a function call on
 * the ~99% of accesses that aren't memory-mapped I/O. */
...
{                                                                       \
        ...
        _lo = (_a < 0xa000) ? ((MY_LITTLE_Z80 *)context)->memory[_a]    \
                            : machine_rd((MY_LITTLE_Z80 *)context, _a);  \
        _a = (_a + 1) & 0xffff;                                         \
        _hi = (_a < 0xa000) ? ((MY_LITTLE_Z80 *)context)->memory[_a]    \
                            : machine_rd((MY_LITTLE_Z80 *)context, _a);  \
        (x) = _lo | (_hi << 8);                                         \
}
...
#define WRLOG(C,_a,_x) wr_log_hook((_a),(unsigned char)(_x),((MY_LITTLE_Z80*)(C))->state.pc & 0xffff)
#else
